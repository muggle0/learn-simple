...
## 八年
![](2022-06-11-00-46-09.png)
![](2022-06-11-00-46-38.png)
![](2022-06-11-00-47-15.png)
![](2022-06-11-00-48-00.png)

## 六年
![](2022-06-11-00-48-39.png)
![](2022-06-11-00-49-48.png)
![](2022-06-11-00-50-19.png)

## 三年
![](2022-06-11-00-51-16.png)
![](2022-06-11-00-51-50.png)
![](2022-06-11-00-52-16.png)

## 七年 
![](2022-06-11-00-53-35.png)
![](2022-06-11-00-54-03.png)
![](2022-06-11-00-54-19.png)


# 正则
https://rgb-24bit.github.io/blog/2018/regex.html

## webull 一面

自我介绍，然后问统一消息业务，描述业务流程，问为什么要双队列。回答双队列可以撤回未发送的消息，还有就是可以对发送队列进行细粒度的流控。

问kafka的的消息可靠传递，三种语义，ack机制。消息丢失问题回答的不好，应该回答ack机制 和isr列表。选举机制 在zk 创建临时节点



 Minor GC时增加了T1（扫描时间），但省去T2（复制对象）的时间，更重要的是对于虚拟机来说，复制对象的成本要远高于扫描成本，所以，单次**Minor GC时间更多取决于GC后存活对象的数量，而非Eden区的大小**。因此如果堆中短期对象很多，那么扩容新生代，单次Minor GC时间不会显著增加。

 ##  dubbo 优雅下线

 runtime 类中 shutdown hook 注册jvm关机钩子，在spring 中是抛出一个容器关闭事件

 应用在停机时，接收到关闭通知时，会先把自己标记为不接受（发起）新请求，然后再等待10s（默认是10秒）的时候，等执行中的线程执行完。

 守护线程调用实现了DisposableBean.destroy 或者AutoCloseable.close。
...
Javassist
  ...
  mysql 对索引的节点进行加锁， next-key lock 行锁加间隙锁。





bossgroup 监听accept事件获取对应的sockchannel 将封装成niosockchannle 绑定channelpipelin，注册channelhandle。绑定到workgorup 的selector ,handler
...
es 文档写入 第一步：客户写集群某节点写入数据，发送请求。（如果没有指定路由/协调节点，请求的节点扮演路由节点的角色。）
...
业务
商品入库，库存表
采购员下采购单-采购管理员审核-运输单-入库单，销售单-出库单
订单表，订单明细，仓库表，仓库明细，仓库操作日志，订单标签
...
● 供应商欠款明细
● 出库利润统计
库存盘点方案-生成盘点清单，出入库单锁单，待盘点。盘点审核，盘点报告；
仓库-仓库明细-仓库id-商品id-商品数量；
营销平台
售后模块：客服用的，物流查询，售后管理；
门店APP模块：导购跟进，用户接待，客户信息，套餐查询
crm 预约活动，平台推广，留资平台，留资线索；
优惠活动查询，用户信息查询，销售单查询，批次单查询
退货单，退货跟踪，责任分析单；

SpringFactoriesLoader 类加载
